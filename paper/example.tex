...
%\caption{An AADL Model with Unbalanced Path\label{motivation1}}
%\end{figure}
%Let us add another thread $E$ after thread $B$ in the previous example. Thread $E$ could be a filter that makes sure no malicous data from thread $B$, a potentially compromised compoent, reaches thread $D$. This kind of  parallel paths merge at a \emph{fusion} thread is quite common in our domain of interest. 